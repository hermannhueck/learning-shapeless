...
object PackExamples extends App {
  ...
  object Pack extends Pack0 {
    ...
    implicit def packHList[F[_], H, T <: HList](implicit fh: F[H], pt: Pack[F, T]): Pack[F, H :: T] = PCons(fh, pt)
  }
  ...
  trait Pack0 {
    ...
    object Unpack extends Unpack0 {
      implicit def unpack1[F[_], H, T <: HList](implicit pc: IsPCons.Aux[F, H :: T, H, T]): Unpack[F, H :: T, H] =
        new Unpack[F, H :: T, H] {
  ...
  implicit val sa: Show[A] =
    new Show[A] { def show = "A" }
  implicit val sb: Show[B] =
    new Show[B] { def show = "B" }
  implicit val sc: Show[C] =
    new Show[C] { def show = "C" }
