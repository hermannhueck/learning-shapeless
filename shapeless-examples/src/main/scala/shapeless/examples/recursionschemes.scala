...
package recursionschemes {
  ...
  object Traverse {
    ...
    implicit val seqTrav: Traverse[Seq] = new Traverse[Seq] {
      ...
      ): G[Seq[B]] = fa.foldRight(G.pure(Seq.empty[B])) { (h, gt) => G.map2(f(h), gt)(_ +: _) }
    }
  }
  ...
  object Morph {
    ...
    object Rec extends Rec1 {
      @annotation.nowarn("msg=Implicit definition should have explicit type")
      implicit def reflexive[T] =
        new Rec[T, T] {
          type F[x] = x
          def tie(t: T): T                                                = t
          def untie(t: T): T                                              = t
    ...
    trait Rec1 extends Rec2 {
      ...
      implicit def generic[T, U, R](
          implicit gen: LabelledGeneric.Aux[U, R],
          R: Rec[T, R]
      ) =
        new Rec[T, U] {
          type F[x] = R.F[x]
