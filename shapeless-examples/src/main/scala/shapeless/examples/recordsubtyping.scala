...
object recordsubtyping extends App {
  ...
  trait default extends Poly1 {
    implicit def id[T]: Case.Aux[T, T] =
      at[T](identity)
  }
  object toUpper extends default {
    implicit def toUpStr: Case.Aux[String, String] =
      at[String](_.toUpperCase)
    implicit def toUpCity: Case.Aux[City, City] =
      at[City](c => new City(c.name.toUpperCase))
    implicit def toUpHl[L <: HList](implicit mv: MapValues[this.type, L]): Case.Aux[L, mv.Out] =
      at[L](mv(_))
  }
